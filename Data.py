...
def create_signal_cluster(data,
                          pixels_per_screen=2000,
                          max_edges_per_screen=5):
    """
    ...
    for index, (zoom, data) in enumerate(cluster):
        ...
        min_duration = min(y[0] - x[0]
                           for x, y in zip(data[:-meps], data[meps:]))
        min_zoom = pixels_per_screen / min_duration
        cluster[index][0] = min_zoom
    # output summary
    if verbose:
        print("Collapsed data set (length %g sec) into the following cluster:"
              % (data[-1][0] - data[0][0]))
...
class BilevelData(Data):
    ...
    def OBSOLETE_find_index_after(self, target_time):
        """Return the first index at or after the given time."""
        if not self.data:
            return None
        low = 0
