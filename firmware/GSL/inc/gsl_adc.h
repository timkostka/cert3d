...
// set the regular sequence to convert the given channel at the given rank
void GSL_ADC_SetRegularSequenceChannel(
    ADC_TypeDef * ADCx,
    uint32_t sequence_rank,
    uint32_t channel) {
  ASSERT_LT(sequence_rank, 16);
  ASSERT_LT(channel, 19);
  auto reg = &ADCx->SQR3;
  reg -= (sequence_rank / 6);
  uint32_t mask = 0b11111 << (sequence_rank % 6);
  *reg &= ~mask;
  *reg |= channel << (sequence_rank % 6);
}
...
#define GSL_ADC_OUTPUT_REGISTER_VALUE(reg, name) \
    GSL_GEN_OutputRegister( \
        TOSTRING(name), \
        ADCx->reg, \
        ADC_##reg##_##name##_Msk, \
        reset_value)

void GSL_ADC_LogDetailedInformation(ADC_TypeDef * ADCx) {
  LOG("\n\n",
      (ADCx == ADC1) ? "ADC1" :
      (ADCx == ADC2) ? "ADC2" : "ADC3",
      " detailed description:");
  uint32_t reset_value = 0;
  if (ADCx->SR != reset_value) {
    LOG("\n- SR:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(SR, OVR);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SR, STRT);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SR, JSTRT);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SR, JEOC);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SR, EOC);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SR, AWD);
  }
  if (ADCx->CR1 != reset_value) {
    LOG("\n- CR1:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, OVRIE);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, RES);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, AWDEN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, JAWDEN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, DISCNUM);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, JDISCEN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, DISCEN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, JAUTO);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, AWDSGL);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, SCAN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, JEOCIE);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, AWDIE);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, EOCIE);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR1, AWDCH);
  }
  if (ADCx->CR2 != reset_value) {
    LOG("\n- CR2:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, SWSTART);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, EXTEN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, EXTSEL);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, JSWSTART);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, JEXTEN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, JEXTSEL);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, ALIGN);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, EOCS);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, DDS);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, DMA);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, CONT);
    GSL_ADC_OUTPUT_REGISTER_VALUE(CR2, ADON);
  }
  if (ADCx->SMPR1 != reset_value) {
    LOG("\n- SMPR1:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP18);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP17);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP16);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP15);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP14);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP13);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP12);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP11);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR1, SMP10);
  }
  if (ADCx->SMPR2 != reset_value) {
    LOG("\n- SMPR2:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP9);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP8);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP7);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP6);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP5);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP4);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP3);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP2);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP1);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SMPR2, SMP0);
  }
  if (ADCx->JOFR1 != reset_value) {
    LOG("\n- JOFR1:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JOFR1, JOFFSET1);
  }
  if (ADCx->JOFR2 != reset_value) {
    LOG("\n- JOFR2:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JOFR2, JOFFSET2);
  }
  if (ADCx->JOFR3 != reset_value) {
    LOG("\n- JOFR3:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JOFR3, JOFFSET3);
  }
  if (ADCx->JOFR4 != reset_value) {
    LOG("\n- JOFR4:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JOFR4, JOFFSET4);
  }
  if (ADCx->HTR != reset_value) {
    LOG("\n- HTR:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(HTR, HT);
  }
  if (ADCx->LTR != reset_value) {
    LOG("\n- LTR:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(LTR, LT);
  }
  if (ADCx->SQR1 != reset_value) {
    LOG("\n- SQR1:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR1, L);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR1, SQ16);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR1, SQ15);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR1, SQ14);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR1, SQ13);
  }
  if (ADCx->SQR2 != reset_value) {
    LOG("\n- SQR2:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR2, SQ12);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR2, SQ11);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR2, SQ10);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR2, SQ9);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR2, SQ8);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR2, SQ7);
  }
  if (ADCx->SQR3 != reset_value) {
    LOG("\n- SQR3:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR3, SQ6);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR3, SQ5);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR3, SQ4);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR3, SQ3);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR3, SQ2);
    GSL_ADC_OUTPUT_REGISTER_VALUE(SQR3, SQ1);
  }
  if (ADCx->JSQR != reset_value) {
    LOG("\n- JSQR:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JSQR, JL);
    GSL_ADC_OUTPUT_REGISTER_VALUE(JSQR, JSQ4);
    GSL_ADC_OUTPUT_REGISTER_VALUE(JSQR, JSQ3);
    GSL_ADC_OUTPUT_REGISTER_VALUE(JSQR, JSQ2);
    GSL_ADC_OUTPUT_REGISTER_VALUE(JSQR, JSQ1);
  }
  if (ADCx->JDR1 != reset_value) {
    LOG("\n- JDR1:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JDR1, JDATA);
  }
  if (ADCx->JDR2 != reset_value) {
    LOG("\n- JDR2:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JDR2, JDATA);
  }
  if (ADCx->JDR3 != reset_value) {
    LOG("\n- JDR3:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JDR3, JDATA);
  }
  if (ADCx->JDR4 != reset_value) {
    LOG("\n- JDR4:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(JDR4, JDATA);
  }
  if (ADCx->DR != reset_value) {
    LOG("\n- DR:");
    GSL_ADC_OUTPUT_REGISTER_VALUE(DR, DATA);
  }
  // Note: we do not output ADC common registers
}
